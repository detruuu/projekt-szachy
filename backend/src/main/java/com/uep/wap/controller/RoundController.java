...
public class RoundController {
    ...
    public ResponseEntity<?> generateNextRound(@PathVariable Long tournamentId) {
        return roundService.generateNextRound(tournamentId)
                .<ResponseEntity<?>>map(round -> ResponseEntity.status(HttpStatus.CREATED).body(round))
                .orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND).body("Tournament not found: " + tournamentId));
    }
}